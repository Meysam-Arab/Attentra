...
elFinder.prototype.view = function(fm, el) {
	...
	*/
	this.renderRow = function(f, odd) {
		...
		return '<tr key="'+f.hash+'" class="'+self.mime2class(f.mime)+(odd ? ' el-finder-row-odd' : '')+'"><td class="icons"><p>'+str+'</p></td><td>'+f.name+'</td><td>'+self.formatPermissions(f.read, f.write, f.rm)+'</td><td>'+self.formatDate(f.date)+'</td><td class="size">'+self.formatSize(f.size)+'</td><td>'+self.mime2kind(f.link ? 'symlink' : f.mime)+'</td></tr>';
	}
...
elFinder.prototype.ui = function(fm) {
	...
	this.showMenu = function(e) {
		...
		this.menu.css({
				right : ((e.clientX + size.cW) > size.width ? ( e.clientX - size.cW) : e.clientX),
			...
			.hover(
				function() { 
					...
					if (s.length) {
						if (!s.attr('pos')) {
							w  = t.outerWidth();
							s.css($(window).width() - w - t.offset().right > s.width() ? 'right' : 'left', w-5).attr('pos', true);
						}
						s.show();
					}
				}, 
...
elFinder.prototype.ui.prototype.commands = {
	...
	open : function(fm) {
		...
		this.exec = function(dir) {
			...
			function openFile(f) {
				...
				window.open(f.url||self.fm.options.url+'?cmd=open&current='+(f.parent||self.fm.cwd.hash)+'&target='+(f.link||f.hash), false, 'top=50,right=50,'+ws+'scrollbars=yes,resizable=yes');
			}
		}
	...
	rm : function(fm) {
		...
		this.exec = function() {
			...
			if (ids.length) {
				this.fm.lockShortcuts(true);
				$('<div><div class="ui-state-error " style="padding:10px;"><span class="ui-icons ui-icons-alert"/><strong>'+this.fm.i18n('Are you sure you want to remove files?<br /> This cannot be undone!')+'</strong></div></div>')
					.dialog({
	...
	upload : function(fm) {
		...
		this.exec = function() {
			...
			var id = 'el-finder-io-'+(new Date().getTime()),
				e = $('<div class="ui-state-error"><span class="ui-icons ui-icons-alert"/><div/></div>'),
				m = this.fm.params.uplMaxSize ? '<p>'+this.fm.i18n('Maximum allowed files size')+': '+this.fm.params.uplMaxSize+'</p>' : '',
				b = $('<p class="el-finder-add-field"><span class="ui-state-default "><em class="ui-icons ui-icons-circle-plus"/></span>'+this.fm.i18n('Add field')+'</p>')
	...
	/**
	 * @class Switch elFinder into icons view
	 * @param Object  elFinder
	 **/
	icons : function(fm) {
	...
	help : function(fm) {
		...
		this.exec = function() {
			...
			h += '<p>'+(ht != 'helpText' ? ht : 'elFinder works similar to file manager on your computer. <br /> To make actions on files/folders use icons on top panel. If icons action it is not clear for you, hold mouse cursor over it to see the hint. <br /> Manipulations with existing files/folders can be done through the context menu (mouse left-click).<br/> To copy/delete a group of files/folders, select them using Shift/Alt(Command) + mouse right-click.')+'</p>';
			h += '<p>'
			...
			a = '<div class="el-finder-help-std"/>'
				...
				+'<p>'+this.fm.i18n('Copyleft: <a href="http://www.std42.ru" target="_blank">Studio 42 LTD</a>')+'</p>'
...
elFinder.prototype.quickLook = function(fm, el) {
	...
	this.win     = $('<div class="el-finder-ql"/>').hide()
		.append($('<div class="el-finder-ql-drag-handle"/>').append($('<span class="ui-icons ui-icons-circle-close"/>').click(function() { self.hide(); })).append(this.title))
		.append(this.ico)
		.append(this.media)
...
elFinder.prototype.eventsManager = function(fm, el) {
	...
	this.init = function() {
		...
		$(document).bind($.browser.mozilla || $.browser.opera ? 'keypress' : 'keydown', function(e) {
			...
			switch(e.keyCode) {
				/* arrows right/up. with Ctrl - exec "back", w/o - move selection */
				case 37:
				case 38:
					e.stopPropagation();
					e.preventDefault();
					...
					break;
				/* arrows left/down. with Ctrl - exec "open", w/o - move selection  */
				case 39:
				case 40:
					e.stopPropagation();
					e.preventDefault();
