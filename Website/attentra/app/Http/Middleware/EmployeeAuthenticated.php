...
//        Log::info('inside __construct in user repository:'.json_encode($this->user));
    ...
    {
        if (auth()->check()&&(auth()->user()->user_type_id == $this->userTypeRepo->getAdminCode()||auth()->user()->user_type_id == $this->userTypeRepo->getCEOCode()||auth()->user()->user_type_id == $this->userTypeRepo->getMiddleCEOCode()||auth()->user()->user_type_id == $this->userTypeRepo->getEmployeeCode())) {
