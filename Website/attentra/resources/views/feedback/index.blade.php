...
@section('content')
    ...
    <div >
        ...
        $columns = [
             ...
             new Column('title','عنوان'),

            (new Column('remove', 'حذفیات'))->setValueFormatter(function($value, $row) {
                return "<a href='feedback/delete/{$row->feedback_id}/{$row->feedback_guid}'>remove</a>";
            }),

            (new Column('details', 'جزییات'))->setValueFormatter(function($value, $row) {
                return "<a href='feedback/show/{$row->feedback_id}/{$row->feedback_guid}'>details</a>";
            }),
