...
@section('content')
    ...
    <div class="alert alert-info  alert-dismissable fade in" style="text-align:center;display:block;margin-top:10px;">
        ...
        <ul>
            <li>محاسبه ساعات کاری کارمندان {{session('companiesName'.$Company_name_index)}} به صورت پیشفرض یک ماه گذشته </li>
        </ul>
    </div>
    <div class="alert alert-danger faa-horizontal animated alert-dismissable fade in" style="text-align:center;display:block;margin-top:10px;">
        <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
        <ul>
            <li>در حال حاضر در این قسمت فقط مجموعه ساعات حضور هر یک از کارمندان به صورت پیش فرض در یک ماه گذشته نمایش داده می شود.
                <br>  جهت مشاهده لیست کارمندان هر نهاد به قسمت مدیریت نهاد ها مراجعه نمائید
                <br> جهت مشاهده لیست ردیابی کارمندان هر نهاد به قسمت مدیریت نهاد ها مراجعه نمائید
                <br> جهت مشاهده لیست ماموریت های کارمندان هر نهاد به قسمت مدیریت ماموریت ها مراجعه نمائید
                <br> جهت مشاهده لیست حضور غیاب کارمندان هر نهاد به قسمت مدیریت حضور غیاب  ها مراجعه نمائید
        ...
        <form action="{{ url('attendance/reports') }}"  method="post">
            ...
            <div class="form-group" style="direction: ltr;">
                ...
                <div class="input-group col-lg-1" style="margin:0px 20px 0px 20px;float:right;">
                    <input type="submit"  class="btn btn-success" id="clacilateWork" value="محاسبه ساعت کاری" placeholder="" />
                </div>
            </div>
        </form>
        ...
        <div class="table-responsive">
            <table id="example" class="display nowrap full-width" cellspacing="0" width="100%">
                ...
                <tr>
                    ...
                    <th>@lang('messages.lbl_FromDate') <span class="fa fa-hourglass"></span></th>

                    <th>@lang('messages.lbl_ToDate') <span class="fa fa-hourglass"></span></th>
                    <th>مجموع ساعات کاری <span class="fa fa-list"></span></th>
                ...
                @foreach($UserRepositories as $user)
                        ...
                        $temp=\Morilog\Jalali\jDateTime::strftime('Y/m/d', strtotime($start));
                        $startconvertedTodate=\Morilog\Jalali\jDateTime::convertNumbers($temp);

                        $temp=\Morilog\Jalali\jDateTime::strftime('Y/m/d', strtotime($end));
                        $endconvertedTodate=\Morilog\Jalali\jDateTime::convertNumbers($temp);
                        ?>
                        <tr>
                            <th>{{$user->name}}</th>
                            <th>{{$user->family}}</th>
                            ...
                            <td>
                                {{$startconvertedTodate}}
                            </td>

                            <td style="text-align: center">
                                {{$endconvertedTodate}}
                            </td>
                            <td style="text-align: center">{{$user->hours}}
                            </td>
                            @if(Auth::user()->user_type_id ==1)
